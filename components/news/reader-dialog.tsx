...
import { getFeedItemContent } from "@/lib/actions/feeds"
...
export function ReaderDialog({
  ...
  useEffect(() => {
    ...
    start(async () => {
      try {
        const data = await getFeedItemContent(item.id)
        setUrl(data.url)
        if (data.content) {
          ...
          setFallback(
            data.summary ||
              "This feed only provides a short excerpt — open the original to read it."
          )
        }
      } catch (error) {
